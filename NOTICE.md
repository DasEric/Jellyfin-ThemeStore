...
The separate [File Transformation](https://github.com/IAmParadox27/jellyfin-plugin-file-transformation) plugin by IAmParadox27 can optionally perform the web-client injection on compatible Jellyfin versions. It is not bundled with this repository and remains under its own GPL-3.0 license. On Jellyfin 12, Theme Store uses its built-in idempotent injector because the current File Transformation release is not compatible with .NET 10.
